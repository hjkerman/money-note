...
따라서 월급 입금을 현금흐름에 기록하면서 기본 예정 수입도 유지하면, 사용자가 기대하는 예산 모델에 따라 수입을 중복으로 본 것처럼 느낄 수 있다. 전체 기간 현금흐름 누계를 계속 쓰는 이유도 현재 코드에 별도 기준월/초기 잔액 모델이 없기 때문이다. 이번 1단계에서는 기존 계산 결과를 보존하며 정책을 바꾸지 않는다.
