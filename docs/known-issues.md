...
현재 Summary는 전체 기간 실제 현금흐름 누계와 아직 들어오지 않은 다음 급여 `scheduled_income`을 함께 사용한다. 월마감은 예정액을 다음 달 1일자 실제 `급여` 현금흐름으로 확정하고, 설정값은 다시 그 다음 급여를 선반영한다. 이는 다음 급여를 담보로 현재 카드 사용을 감당하는 현행 운용에 맞춘 의도적인 계산이다.
