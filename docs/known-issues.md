...
## 예정 수입과 현금흐름 기준의 의미 정리 필요

현재 Summary의 잔여 유동성은 `기본 예정 수입 + 전체 기간 현금흐름 누계`를 함께 사용한다. 현금흐름의 `이달 기준 수입` 표시는 기본 예정 수입을 대체하지 않고 카드 결제 압박 Judgment의 기준 수입으로만 쓰인다.

따라서 월급 입금을 현금흐름에 기록하면서 기본 예정 수입도 유지하면, 사용자가 기대하는 예산 모델에 따라 수입을 중복으로 본 것처럼 느낄 수 있다. 전체 기간 현금흐름 누계를 계속 쓰는 이유도 현재 코드에 별도 기준월/초기 잔액 모델이 없기 때문이다. 유동성 이름 migration에서도 기존 계산 결과를 보존하며 이 정책은 바꾸지 않았다.

향후 다음 수입을 담보로 현재 신용카드를 사용하는 운용을 청산하면 예산 주기는 매월 1일~말일로 고정하고, `scheduled_income`을 매달 시작 재원으로 한 번씩 반영하며, 현금흐름은 해당 월 범위로 집계한다. 카드 결제 압박 Judgment의 기준은 아직 결정하지 않았다. 전환 조건과 영향 범위는 [미래 예산 주기 전환](future-budget-cycle-transition.md)을 따른다.
...
- 2026-08-20 v4 Snapshot을 새 카드 정책 명세 v2로 전환하기 위해 내부 정책 명세 v1 읽기 분기가 남아 있다. 배포 후 v2 Snapshot이 실제로 생성·검증되면 이 분기를 제거한다.
