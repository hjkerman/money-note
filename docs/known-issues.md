...
향후 다음 수입을 담보로 현재 신용카드를 사용하는 운용을 청산하면 카드 결제 압박 Judgment의 분모를 `이달 기준 수입`에서 `remaining_liquidity`로 바꾸는 방안을 검토한다. 전환 조건과 영향 범위는 [미래 예산 주기 전환](future-budget-cycle-transition.md)을 따른다.
