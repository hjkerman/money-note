...
월마감은 사용자가 다음 카드대금에 쓸 자금을 실제로 꺼내는 사건이며, `scheduled_income`을 월마감 실행일의 실제 `급여` 현금흐름으로 확정한다. `scheduled_income` 설정값은 다음 월마감에도 재사용할 기준선으로 남지만 Summary 잔여 유동성에 직접 더하지 않는다.
...
과거에는 설정값과 실현된 급여를 모두 더해 동일 수입을 이중계상했으나 2026-08-31 수정으로 직접 선반영을 제거했다. Judgment 기준은 별도이며 [미래 재무 건전화 전환](future-financial-health-transition.md)을 따른다.
