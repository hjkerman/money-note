...
홈의 예산심사위원회 카드는 기존 Judgment를 표시하는 동시에 ChatGPT 회계감사 공유 진입점이다. 회계감사 자료는 새 API 없이 기존 원장, 현금흐름, 정산, 카드 정기결제 확인 목록 응답을 모바일에서 Markdown으로 조립한다. 이때 서버가 내려준 할인액과 실결제액을 소비하며 도메인 계산식을 복제하지 않는다. 현금성 고정지출, 카드 정기결제 템플릿과 미삭제 동결 항목은 선택 월 기록과 분리된 생성 시점 현재 참고자료로만 제공한다. Android 네이티브 공유층은 보고서를 앱 전용 cache에 임시 생성하고 `com.openai.chatgpt`에만 읽기 URI를 허용한다. Money-Note 복귀 또는 다음 실행 때 임시 파일을 삭제한다.
