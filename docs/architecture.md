...
- 자동 배분은 활성 결제 batch의 일반 카드 사용내역을 날짜순으로 정렬하고 현재 유동성 안에서 앞 항목부터 배분한다. 교통·통행료 항목은 자동 배분하지 않으며, 마지막 일반 항목에는 일부 금액만 배분할 수 있다.
...
- 14일이 지난 뒤에도 기록상 남은 결제금액을 숨기지 않으며, 유동성은 자동 수정하지 않고 사용자의 수동 확인과 보정을 요구한다.
