...
가족 공유 페이지는 본체 로그인과 분리된 `share_sessions` 장기 세션을 사용한다. PIN이 설정되지 않았으면 기존처럼 열리고, PIN 설정 후에는 카카오톡 인앱 브라우저를 포함한 각 브라우저에서 한 번 PIN을 입력해야 한다. PIN 변경 시 기존 공유 세션은 모두 종료된다.
