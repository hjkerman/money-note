...
통행료 자료는 아직 서버 도메인에 편입되지 않은 실험 데이터다. 모바일 설정 맨 아래의 `Experimental Data`에서 고속도로통행료+ `com.ex.hipass_app` 원문을 확인한다.
...
동결 금액은 당장 소비하지 않고 따로 확보해 둔 돈을 관리하는 대기 목록이다.

- 원장이나 월별 소비 기록이 아니다.
- 등록 월이 지나도 자동으로 사라지거나 숨겨지지 않는다.
- 사용자가 개별 항목을 삭제할 때까지 웹과 모바일의 동결 목록에 계속 표시한다.
- 익월 유동성의 동결자산 총액도 같은 전체 미삭제 목록을 기준으로 계산한다.
