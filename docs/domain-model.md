...
예산 주기는 매월 1일부터 말일까지다. 월마감이 성공하면 닫힌 달의 다음 달 1일자로 설정의 `scheduled_income`과 같은 금액의 양수 현금흐름 `급여`를 만들고 `is_primary_income=1`로 표시한다. 월마감 시점의 금액을 확정하므로 이후 설정 변경은 과거 급여 행을 바꾸지 않는다. 다만 발생일이 서버 기준일보다 미래라면 행만 존재할 뿐 `cash_flow_balance`에는 들어가지 않으며, 해당 날짜부터 실제 잔액에 포함된다.

현재 계산식의 별도 `scheduled_income` 항은 아직 현금흐름으로 확정되지 않은 다음 급여를 한 번 선반영한다. 월마감으로 그 급여가 실제 현금흐름이 되면, 설정값은 다시 그 다음 급여의 예정액 역할을 한다. 이는 다음 급여를 담보로 현재 카드 사용을 감당하는 현행 운용을 의도적으로 표현한다.
