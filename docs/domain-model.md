...
알림 원문과 후보는 모바일 로컬에만 저장한다. 후보 생성 또는 `통행료 납치 성공` 알림은 원장, Claim, Family Card, 카드대금, Snapshot에 영향을 주지 않는다. 사용자가 후보를 확인하고 `등록`을 눌러 기존 서버 API가 성공한 시점부터 서버 데이터가 된다. 통행료 후보는 할인 제외로 등록한다.
