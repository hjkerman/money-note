...
정책 선택과 실결제액 계산의 단일 진실 원천은 `backend/app/services/card_charge/`다. 사용월별 정책 이력을 코드 레지스트리에 보존하므로, 나중에 카드를 바꾸더라도 과거 거래를 새 정책으로 재해석하지 않는다. 정책 교체 UI는 두지 않는다.
...
본인카드와 가족카드의 월별 적용 여부는 서로 독립적으로 저장한다. 통행료카드와 교통카드는 현재 자동 할인 정책 자체가 없으므로 별도 월 스위치를 사용하지 않는다.
