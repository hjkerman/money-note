...
예산 주기는 매월 1일부터 말일까지다. 월마감이 성공하면 닫힌 달의 다음 달 1일자로 설정의 `scheduled_income`과 같은 금액의 양수 현금흐름 `급여`를 만들고 `is_primary_income=1`로 표시한다. 이 행은 실제 입금 기록이며 전체 기간 현금흐름 누계에 남는다. 월마감 시점의 금액을 확정하므로 이후 설정 변경은 과거 급여 행을 바꾸지 않는다.
