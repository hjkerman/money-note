...
청구 또는 가족카드의 현재 전달분을 일괄 처리 완료하고 삭제한다. `claim`, `family_card`만 허용하며 다른 패널과 월마감에는 영향을 주지 않는다.
...
- `family_card_limit`: 가족카드 한도 감시 기준 금액
...
최근 3개월 장부 데이터와 비민감 운영 설정을 JSON snapshot 파일로 내려받는다.
...
- `exported_at`
- `range`
- 최근 3개월 관련 `ledger_entries`, `monthly_panels`, `cash_flows`
- 활성 또는 최근 관련 `installments`
- 관련 `card_payment_events`, `card_payment_allocations`, `card_payment_deferrals`
- 비민감 `app_settings`
- `app_labels`
...
`users`, `auth_sessions`, `share_sessions`, `audit_logs`는 복원 대상이 아니다. snapshot 구조가 맞지 않거나 지원하지 않는 `schema_version`이면 `400`을 반환한다.
