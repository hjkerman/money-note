...
### `GET /api/admin/snapshot/pre-restore/{filename}`

특정 `pre_restore` snapshot 파일을 다운로드한다.

`filename`은 `pre_restore-YYYYMMDDTHHMMSSZ.money-note-snapshot.json` 또는 같은 초에 여러 개가 만들어질 때의 `pre_restore-YYYYMMDDTHHMMSSZ-2.money-note-snapshot.json` 형식만 허용하며, 서버는 `snapshot-backups` 디렉터리 밖의 파일을 읽지 않는다.
