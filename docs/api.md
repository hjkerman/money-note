...
즉시결제를 항목별로 배분한다. `event_type = discount`는 기존 백업/데이터 호환을 위해 남아 있지만, 일반 웹 UI에서는 수기 할인액 입력을 제공하지 않는다.
...
청구 또는 타인정산 항목의 개별 할인 예외를 기록한다. 웹 UI에서는 `할인 제외`를 누르면 할인액 0원, `할인 적용`을 누르면 기본 1.2% 할인 계산으로 돌아간다. 원래 금액은 유지하며 화면과 공유 페이지의 실제 정산액은 `원래 금액 - 할인액`으로 계산한다. 청구는 본인회원 카드 정책을, 타인정산은 가족카드 정책을 따른다.
