...
보고서에는 선택 월의 본인 카드 지출, 현금흐름, 미정산 청구, 미정산 가족카드가 포함된다. 청구 절은 선택 월 사용일에 해당하는 현재 미정산 항목의 건수, 서버 투영 실청구액 합계와 개별 사용일·내용·원금·할인액·실청구액을 표시한다. 본인 원장의 사용금액과 서버 확정 할인액·실결제액을 함께 적고, 청구는 같은 카드 소비의 최종 부담 주체와 회수 관계일 수 있으므로 독립 소비로 중복 합산하지 않으며 Summary나 잔여 유동성을 다시 계산하지 말라는 지침도 파일 안에 포함한다. Claim과 Family Card는 별도 domain으로 표시한다. 처리 완료된 청구와 가족카드는 운영 중 삭제되므로 과거 월 보고서에도 현재 남아 있는 항목만 들어갈 수 있다.
...
- 카드 즉시결제가 만든 현금흐름은 일반 현금흐름 삭제에서 거부된다. 정상 취소는 `이번달 결제`의 이벤트 취소를 사용하며, 이 경로가 결제 이벤트·allocation·현금흐름을 함께 되돌린다.
...
- 월마감 시점의 기본 예정 수입을 **월마감 실행일자** `급여` 입금으로 한 건 기록하고 Active 계좌 잔액에 즉시 반영한다. 이 행은 `이달 기준 수입`이며 같은 급여를 수동으로 다시 입력하지 않는다.
