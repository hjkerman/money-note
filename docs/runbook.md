...
모바일 앱의 `알림에서 가져오기`는 우리카드 승인 알림을 로컬 후보함으로 모으는 실사용 화면이다. 우리카드 원문은 `설정 -> 최근 우리카드 알림`에서 단독으로 확인한다. 교통 관련 실험 자료는 설정 메뉴 맨 아래 `Experimental Data`의 `교통 / 통행` 탭으로 격리한다.
...
- `최근 우리카드 알림`과 `Experimental Data`에서 원문을 개별 또는 출처별 파일로 공유할 수 있다.
...
현재 고속도로통행료+ 패키지는 `com.ex.hipass_app`, 모바일티머니 패키지는 `com.lgt.tmoney`로 확인되어 있다. 향후 앱 교체나 패키지 변경이 의심되면 설치된 실기기에서 확인한다.
...
2. `Experimental Data -> 교통` 탭에서 원문 필드를 확인하고 공유한다.
...
모바일티머니 후보 생성이 실사용에 충분히 안정되면 `교통` 기능을 `Experimental Data`에서 승격하고 `최근 우리카드 알림` 관측 화면과 통합할 수 있다. 승격 전에는 실험 자료를 우리카드 후보함이나 서버 API와 섞지 않는다.
