...
- 조기 월마감으로 다음 달 1일자 급여 행을 미리 만들어도 발생일 전에는 `cash_flow_balance`에 포함하지 않는다. 발생일부터 실제 잔액에 들어가고 이후 누계에 남는다.
...
월마감이 만든 `급여`는 발생이 확정된 미래 현금흐름 행이므로 Snapshot, pre_restore, 초기화, 현금흐름 조회와 삭제의 기존 규칙을 그대로 따른다. 발생일 전에는 Active 계좌 잔액이 아니다. 별도 누적 설정이나 DB 컬럼은 두지 않는다.
...
- future-dated cash flow가 발생일 전 Active 계좌 잔액에 들어가지 않는지 확인한다.
