...
- **현금성 고정지출 확인**: 확인 전에는 미지급 의무로 잔여 유동성에 반영하고, 확인 시 오늘 이하 실제 처리일의 음수 현금흐름으로 원자적으로 전환한다. 확인 주기를 별도로 기록해 밀린 과거 월마감이 이후 주기 상태를 reset하지 않는다. 표시용 정기지출 총액은 줄지 않으며 월마감 후 대상 주기 템플릿만 다시 활성화된다.
...
- **Active 계좌 잔액**: `cash_flow_balance`는 수동 보정값과 서버 기준일까지 실제 발생한 현금흐름 누계다. 미래 날짜 급여 행은 존재하더라도 발생일 전에는 잔액에 포함하지 않는다.
