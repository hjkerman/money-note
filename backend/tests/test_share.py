...
class SharePanelTest(unittest.TestCase):
    def setUp(self) -> None:
        ...
        with session() as conn:
            ...
            conn.execute(
                """
                INSERT INTO monthly_panels(month, panel_type, title, spent_on, amount_value, sort_order)
                VALUES
                    ('2026-06', 'claim', '지난달 병원비', '2026-06-22', 1000, 1),
                    ('2026-07', 'claim', '이번 달 커피', '2026-07-02', 2000, 2),
                    ('2026-07', 'claim', '전세대출이자', '2026-07-03', 300, 3)
                """
            )
    ...
    def test_shared_panel_reports_minimum_payment_total(self) -> None:
        ...
        self.assertEqual(data["minimum_total"], 1300)
        self.assertEqual(data["total"], 3300)
    ...
    def test_shared_panel_html_dims_current_month_non_interest_rows(self) -> None:
        ...
        self.assertIn("최소 결제 합계 1,300원", html)
