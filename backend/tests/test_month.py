...
class MonthCloseTest(unittest.TestCase):
    ...
    def test_close_realizes_income_and_prefunds_the_next_scheduled_income(self) -> None:
        with session() as conn:
            conn.execute("DELETE FROM ledger_entries WHERE entry_kind = 'planned'")
        ...
        self.assertEqual(before["remaining_liquidity"], 400_000)
        self.assertEqual(after["scheduled_income"], 400_000)
        self.assertEqual(after["cash_flow_balance"], 400_000)
        self.assertEqual(after["remaining_liquidity"], 800_000)
