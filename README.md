...
서버 DB와 서버 API 계산 결과가 단일 진실 원천입니다. 과거의 스프레드시트 운용 방식에서 출발했지만, 현재 앱은 웹 UI와 모바일 앱이 같은 API를 사용하는 구조입니다.
...
- 모바일 앱: Flutter 기반 Android 실사용 클라이언트. 빠른 입력, 현금흐름, 당월 내역, 정산과 운영 설정에 집중
- 운영 경계와 문서 읽기 순서: [프로젝트 상태](docs/project-state.md)
...
- 할인·유동성·요약 계산은 서버에만 두고 클라이언트에서 복제하지 않는다.
...
- 동결: 실제로 보유하지만 현재 가용 유동성에서 제외한 금액. 사용자가 삭제할 때까지 월 경계와 무관하게 유지
- 현금흐름: 현금 입출금 기록을 현금흐름 반영액과 잔여 유동성에 반영
- 카드 할인: 본인·가족 월별 혜택과 교통카드의 본인카드 정책 추종 여부를 설정. 통행료는 자동 할인 없음, 개별 항목은 수동 실결제액 보정 가능
- 이번달 결제: 마지막 월마감이 만든 활성 결제 batch를 날짜순으로 자동 배분하거나 직접 선택해 일부 즉시결제
- 카드 사용내역 이월: 결제 화면에서 항목을 일부결제하거나 다음 달 장부 맨 앞으로 이월
- 수동 월마감: 새 달 기록을 먼저 입력해도 가장 오래된 미마감 월 하나만 전체 기록으로 이동하며, 대상 월 재시도에도 archive·급여·결제 batch를 중복 생성하지 않음
...
- Snapshot 백업/복원: 장부 운용 데이터 전체와 비민감 운영 설정을 단일 DB 시점에서 읽어 SHA-256 manifest가 포함된 JSON snapshot으로 보관하고, mandatory pre_restore와 dry-run 뒤 복원
