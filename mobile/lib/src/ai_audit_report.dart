...
class AiAuditReportData {
  ...
  String buildMarkdown(String month, {DateTime? generatedAt}) {
    ...
    return [
      ...
      '- 기본 예정 수입 설정값 자체는 현재 자산이나 잔여 유동성에 직접 포함되지 않습니다.',
      '- 월마감 실행일에 같은 금액의 실제 급여 현금흐름이 생성되면 Active 계좌 잔액과 잔여 유동성에 한 번 반영됩니다.',
      '- 잔여 유동성은 서버가 계산한 현재 값이며, 기준 월 수입을 별도로 더해 다시 계산하지 않습니다.',
      '',
      '## 현재 운영 참고자료',
      '',
